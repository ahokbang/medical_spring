...
<section>
    ...
    <table>
      ...
      <tr>
      	<%-- <td colspan="4"> 총 개수 : ${countAll } 개</td> --%>
      	<td colspan="4"> 총 개수 : ${map.countAll } 개</td> <%--  map으로 보낼 땐 이렇게 해야해 --%> 
      </tr>
      <!-- 제목부분 -->
      <tr>
        <th>No.</th>
      ...
      <c:forEach items="${map.list}" var="bdto">   <%--  map으로 할 때--%>
	      <tr>
	...
	<ul class="page-num">
      ...
      <!-- prev 부분 -->
      <c:if test="${map.page<=1}"><li class="prev"></li>
      </c:if>
      <c:if test="${map.page>1}">
      ...
      <c:if test="${map.page>=map.maxPage}"><li class="next"></li>
      </c:if>
      
      <c:if test="${map.page<map.maxPage}">
      	<a href="list?page=${map.maxPage}"><li class="last"></li></a>
      </c:if>
      <c:if test="${map.page>=map.maxPage}"><li class="last"></li>
      </c:if>
    </ul>

<%--     <ul class="page-num"> <!-- 데이터 개수가 중요. 데이터 개수 나누기 5 또는 10하면 페이지 수 -->
    <!-- first  -->
    <c:if test="${map.page<=1}">
      <li class="first"></li>
     </c:if>
