	...
	<body>
		...
		<form action="doStudents" name="sfrm" method="post">
			...
			<input type="text" name="math" value="${stu.math }"> <br> 
  			...
  			<label>성별</label><br>
			<input type="radio" name="gender" id="male" value="male" <c:if test="${fn:contains(stu.gender,'male')}">checked</c:if>>  <!-- id는 label에 넘겨주는 값이고 value는 input하지 않고 선택하니까 value를 적어주는 것 -->
			<label for="male">남자</label>
			<input type="radio" name="gender" id="female" value="female" <c:if test="${fn:contains(stu.gender,'female')}">checked</c:if>> 
